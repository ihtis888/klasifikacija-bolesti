...
public class Test {
	...
	public void runCommand() {
		...
		*/
		...
		try {
			...
			int brNeurona;
			int brEksperimenta=0;
			double stopaUcenja = 0.1;
			
			int typedEpochs = 1000;
			int typedNumHdnLayer = 0;
			double typedLearningRate = 0;	
			
			
			for(brNeurona=1; brNeurona<6; brNeurona++) {
				for(stopaUcenja=0; stopaUcenja < 0.9; ) {
					stopaUcenja *= 10;
				    stopaUcenja = stopaUcenja + 1;
				    stopaUcenja /=10;

				    brEksperimenta++;
					
					stopaUcenja = round(stopaUcenja, 2);
					
					setBrExperimenta(Integer.valueOf(lblBrojacEksperimenta.getText()) + 1);
					lblBrojacEksperimenta.setText(getBrExperimenta().toString());
					
					typedNumHdnLayer = brNeurona;
					typedLearningRate = stopaUcenja;		
					
					DiagnosisExample test = new DiagnosisExample();
					test.setTxtRezultat(txtRezultat);
					test.setPanelErrorEvolution(panelErrorEvolution);

					uspjesno = test.pokreniTest2(option, typedEpochs, typedNumHdnLayer, typedLearningRate, numberOfInputs, numberOfOutputs, inputColumns, outputColumns, getBrExperimenta(), dataSet);
					
					nazivSlike = option + " " + Integer.toString(brEksperimenta) + " brNeur=" + brNeurona+ " stopaUcenja=" + Double.toString(stopaUcenja) + " uspjesno=" + uspjesno;
					savePictureToFile(panelErrorEvolution,"C:\\temp\\",nazivSlike);
					panelErrorEvolution.removeAll();
				...
				while(koraci < 100) {
					koraci = koraci + 10;
				   // stopaUcenja = koraci / 100;
				    
				    stopaUcenja *= 10;
				    
				    stopaUcenja = stopaUcenja + 10;
		...
		//		}
				...
				//for(stopaUcenja=0.1; stopaUcenja < 1; stopaUcenja = stopaUcenja + 0.1) {
				//	brEksperimenta++;
					/*
					stopaUcenja = round(stopaUcenja, 2);
	...
	public void savePictureToFile(JPanel panel, String putanja, String detaljiEksperimenta) {
		File file = null;
		/*JFileChooser fileChooser = new JFileChooser();
		if (fileChooser.showSaveDialog(panelErrorEvolution) == JFileChooser.APPROVE_OPTION) {
		  file = fileChooser.getSelectedFile();
		...
		file = new File(putanja + detaljiEksperimenta + ".jpg");
	    BufferedImage img = new BufferedImage(panel.getWidth(), panel.getHeight(), BufferedImage.TYPE_INT_RGB);
	    panel.print(img.getGraphics()); // or: panel.printAll(...);
	    try {
	        ImageIO.write(img, "jpg", file);
