...
public class DataNormalization {
	...
	public double[] normalize(double[] data) {
		...
		for (int i = 0; i < rows; i++) {
			switch (TYPE) {
			case MIN_MAX:
				normalizedData[i] = (minNorm)
						+ ((data[i] - minValues[0]) / (maxValues[0] - minValues[0])) * (scaleNorm);
				break;
			case ZSCORE:
	...
	public double[][] denormalize(double[][] normalizedData) {
		...
		for (int i = 0; i < rows; i++) {
			for (int j = 0; j < cols; j++) {
				switch (TYPE) {
				case MIN_MAX:
					denormalizedData[i][j] = minValues[j]
							+ (((normalizedData[i][j] - minNorm) / scaleNorm) * (maxValues[j] - minValues[j]));
					break;
				case ZSCORE:
	...
	public double[] denormalize(double[] normalizedData, int column) {
		...
		for (int i = 0; i < rows; i++) {
			switch (TYPE) {
			case MIN_MAX:
				denormalizedData[i] = minValues[column]
						+ (((normalizedData[i] - minNorm) / scaleNorm) * (maxValues[column] - minValues[column]));
				break;
			case ZSCORE:
