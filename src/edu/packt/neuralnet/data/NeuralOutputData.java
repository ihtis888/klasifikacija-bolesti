...
public class NeuralOutputData {
    ...
    public void calculatePerformanceMeasures(double[][] confMat, StyledDocument document ) {
		// positive class error rate
		double errorRatePositive = confMat[0][1] / (confMat[0][0]+confMat[0][1]);
		// negative class error rate
		double errorRateNegative = confMat[1][0] / (confMat[1][0]+confMat[1][1]);
		...
		try {
			...
			document.insertString(document.getLength(), "stopa greške pozitivne klase: "+(errorRatePositive*100.0)+"%\n", null); //positive class error rate
			document.insertString(document.getLength(), "stopa greške negativne klase: "+(errorRateNegative*100.0)+"%\n", null); //negative class error rate
			document.insertString(document.getLength(), "ukupna stopa greške: "+(totalErrorRate*100.0)+"%\n", null);	//total error rate
			document.insertString(document.getLength(), "ukupna tačnost: "+(totalAccuracy*100.0)+"%\n", null); //total accuracy
			document.insertString(document.getLength(), "preciznost: "+(precision*100.0)+"%\n", null);  //precision
			document.insertString(document.getLength(), "osjetljivost: "+(sensibility*100.0)+"%\n", null); //sensibility
			document.insertString(document.getLength(), "specifičnost: "+(specificity*100.0)+"%\n", null); //specificity
